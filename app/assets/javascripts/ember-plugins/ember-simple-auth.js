...
Ember.SimpleAuth = Ember.Namespace.create({
  ...
  **/
  setup: function(container, application, options) {
    ...
    var authorizer = (options.authorizer || Ember.SimpleAuth.Authorizers.Devise).create({ session: session });
...
Ember.SimpleAuth.Authorizers.Devise = Ember.SimpleAuth.Authorizers.Base.extend({
  /**
    Authorizes an XHR request by adding the `access_token` property from the
    session as a bearer token in the `Authorization` header:
...
Ember.SimpleAuth.Authenticators.Devise = Ember.SimpleAuth.Authenticators.Base.extend({
  ...
  */
  restore: function(properties) {
    ...
    return $.ajax({
      ...
      async: false,
      type: "POST",
      data: { remember_token: properties.remember_token },
      dataType: "json",
      contentType: "application/x-www-form-urlencoded",
      success: function(response){
        return Ember.$.extend(response, {
  ...
  */
  ...
  authorize: function(jqXHR, options) {
    return jqXHR.setRequestHeader('X-auth-token', this.get('session.auth_token'));
  },
  ...
  actions: {
    ...
    */
    authenticate: function() {
      var data = this.getProperties('identification', 'password', 'remember_me');
